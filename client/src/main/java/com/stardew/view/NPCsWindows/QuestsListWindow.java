...
public class QuestsListWindow extends CloseableWindow {
    public QuestsListWindow(int gameId, Stage stage, NPCType npc) {
        ...
        NPCController.getQuestsList(npc, questDescriptions -> NPCController.getRelationWithNPC(gameId, npc,
            ...
            Label titleLabel = new Label(npc.getName() + " Quests:", GamePictureManager.skin);
            ...
            NPCController.getNPCQuestsStatus(gameId, npc, questsStatus -> {
                for (int i = 0; i < questDescriptions.size(); i++) {
                    ...
                    if (i == 0) {
                        isDone = questsStatus.getFirst();
                    } else if (i == 1) {
                        isLocked = relationWithNPCDTO.isSecondQuestLocked();
                        isDone = questsStatus.get(1);
                    } else if (i == 2) {
                        isLocked = relationWithNPCDTO.isThirdQuestLocked();
                        isDone = questsStatus.get(2);
                    }
                    ...
                    Image lockIcon = new Image(isLocked ? GamePictureManager.lockedIcon :
                    ...
                    doButton.addListener(new ClickListener() {
                        ...
                        public void clicked(InputEvent event, float x, float y) {
                            NPCController.doQuest(gameId, npc, index, result -> {
                                showResult(result);
                                closeWindow();
                            });
                        }
