...
public class FriendshipLevelWithNPCWindow extends CloseableWindow {
    ...
    public FriendshipLevelWithNPCWindow(int gameId, Stage stage, NPCType npc) {
        super("Friendship with " + npc.getName(), stage);

        NPCController.getRelationWithNPC(gameId, npc, relation -> {
            float fontScale = 1.2f;
            Color fontColor = Color.BLACK;
            int maxXp = npc.getMaxFriendShipLevel();
            Table contentTable = new Table();
            contentTable.defaults().pad(8).left();
            Label levelLabel = new Label("Friendship Tier:      " + relation.getNpcFriendshipLevel().toString(),
                GamePictureManager.skin);
            levelLabel.setFontScale(fontScale);
            levelLabel.setColor(fontColor);
            Label xpLabel =
                new Label("Friendship XP:       " + relation.getNumericalFriendShipLevel() + " / " + maxXp,
                    GamePictureManager.skin);
            ...
            Image secondQuestIcon = new Image(relation.isSecondQuestLocked() ? GamePictureManager.lockedIcon :
                GamePictureManager.unlockedIcon);
            ...
            Image thirdQuestIcon = new Image(relation.isThirdQuestLocked() ? GamePictureManager.lockedIcon :
                GamePictureManager.unlockedIcon);
            ...
            Image spokenIcon = new Image(relation.isFirstTimeToSpeakWithNPC() ? GamePictureManager.noIcon :
                GamePictureManager.yesIcon);
            ...
            Image giftedIcon = new Image(relation.isFirstTimeGiftToNPC() ? GamePictureManager.noIcon :
                GamePictureManager.yesIcon);
